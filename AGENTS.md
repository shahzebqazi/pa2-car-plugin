...
- **Mockup app (`mockup/`):** Vite + Svelte, **hash routes**: `#/` **home** (concise project overview, framed DHU reference image from `public/dhu/`, centered nav to other sections), `#/design`, `#/research`, `#/mockup` and `#/mockup/<frame>`. **Sandbox — Car App Library** browse and now-playing entries are **Svelte wireframes** (`CarAppLibraryBrowse.svelte`, `CarAppLibraryNowPlaying.svelte`) approximating `androidx.car.app` templates — not embedded DHU screenshots; PNGs under `public/dhu/` stay in the repo for side-by-side comparison and the home hero.
...
- **Label every frame** per `08-mockup-handoff-package.md`: **Phone — PA2 theme** vs **Auto — host media (PA2)** vs **Sandbox — Car App Library** (Svelte mocks aligned to the Kotlin sample under `app/.../car/`, not PA2 shipping Auto).
